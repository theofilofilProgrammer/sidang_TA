{{-- filepath: resources/views/admin-nextPage/mata-kuliah/form-edit-datamatkul.blade.php --}}
...
<div class="content-wrapper col-lg-10 px-0 mx-4">
    <section class="form-data-section container col-lg-12 d-flex flex-column justify-content-center py-5 px-5 mx-0 my-5">
        <div class="col-12 px-0 d-flex flex-column mb-4">
            <h3 class="section-title mb-2">EDIT DATA MATA KULIAH</h3>
        </div>
        ...
        <form method="POST" action="{{ route('matakuliah.update', $matakuliah->id) }}">
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <input type="text" class="form-control @error('kode_mk') is-invalid @enderror" id="kode-matkul" name="kode_mk" value="{{ old('kode_mk', $matakuliah->kode_mk) }}" required>
                    @error('kode_mk') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="form-group col-md-6">
                    <label for="nama-matkul">Nama Mata Kuliah</label>
                    <input type="text" class="form-control @error('nama_mk') is-invalid @enderror" id="nama-matkul" name="nama_mk" value="{{ old('nama_mk', $matakuliah->nama_mk) }}" required>
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-8">
                    ...
                    <select id="keterangan-matkul" name="keterangan" class="form-control @error('keterangan') is-invalid @enderror">
                        <option value="Wajib" {{ old('keterangan', $matakuliah->keterangan) == 'Wajib' ? 'selected' : '' }}>Wajib</option>
                        <option value="Pilihan" {{ old('keterangan', $matakuliah->keterangan) == 'Pilihan' ? 'selected' : '' }}>Pilihan</option>
                        <option value="Tidak Wajib" {{ old('keterangan', $matakuliah->keterangan) == 'Tidak Wajib' ? 'selected' : '' }}>Tidak Wajib</option>
                    </select>
